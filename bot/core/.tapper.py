                ...
                if time() - access_token_created_time >= 3600:
                    ...
                    if daily_combo:
                        ...
                        if not is_claimed:
                            ...
                            # cards = ['p2p_trading', 'gamefi_tokens', 'villa_for_dev_team]
                    ...
                    if settings.AUTO_UPGRADE is True:
                        ...
                        for _ in range(settings.UPGRADES_COUNT):
                            ...
                            for upgrade in available_upgrades:
                                ...
                                significance = max(price, 1) / (profit + earn_on_hour)  
                                ...
                                # logger.info(
                                # f'{self.session_name} | Ціна карточки significance <e>{significance}</e>'
                                # )
                            ...
                            spusok = heapq.nsmallest(10, queue)
                            ...
                            for item in spusok:
                                ...
                                payback = card_list["price"] / card_list["profitPerHourDelta"]

                                payback = payback / 24

                                # timedelta = datetime.timedelta(days=payback)
                                #
                                # days = timedelta.days
                                ...
                                logger.info(
                                    f'{self.session_name} | Name: <g>{card_list["name"]}</g> Lv: <b>{card_list["level"]}</b>  Ціна: <r>{card_list["price"]:,}</r>  Прибутковість: {card_list["profitPerHourDelta"]:,}'
                                )

                            free_money = balance - settings.BALANCE_TO_SAVE

                            top_card_check = top_card[2]

                            top_card_price = top_card_check['price']

                            upgrade = top_card[2]

                            # print('топова карта на оновлення  ', upgrade)
                            ...
                            logger.info(
                                f'{self.session_name} | Бабло в годину: <y>{earn_on_hour:,}</y>'
                            )

                            logger.info(
                                f'{self.session_name} | Вільні кошти: <g>{free_money:,}</g>'
                            )

                            logger.info(
                                f'{self.session_name} | Назва топової карти: <e>{upgrade["name"]}</e> Lv: <r>{level}</r>'
                            )

                            logger.info(
                                f'{self.session_name} | Ціна топової карти: <e>{top_card_price:,}</e>'
                            )

                            logger.info(
                                f'{self.session_name} | Дохід за годину: <e>{upgrade["profitPerHourDelta"]:,}</e>'
                            )

                            # if price > free_money * 0.8:
                            #     logger.error(
                            #         f'{self.session_name} | <r>Не хватає бабла!  Виходимо !!!!</r>'
                            #     )  

                            # else:                             

                            #     logger.success(
                            #         f'{self.session_name} | <g>Є бабло!  Купляємо !!!!</g>'
                            #     )
                            ...
                            await asyncio.sleep(delay=randint(5, 14))
