                    ...
                    if settings.AUTO_UPGRADE is True:
                        ...
                        for _ in range(settings.UPGRADES_COUNT):
                            ...
                            for upgrade in available_upgrades:
                                ...
                                # significance = max(price, 1) / (profit + profit_per_hour)

                                # significance = (profit + profit_per_hour) / max(price, 1)
                            ...
                            logger.info(
                                f'{self.session_name} | На балансі: <g>{free_money:,}</g>'
                            )  

                            logger.info(
                                f'{self.session_name} | Дохід в годину: <g>{earn_on_hour:,}</g>'
                            ) 
                            ...
                            logger.info(
                                f'{self.session_name} | Назва карточки: <e>{name}</e>'
                            )

                            logger.info(
                                f'{self.session_name} | Рівень: <e>{level}</e>'
                            )

                            logger.info(
                                f'{self.session_name} | Ціна: <e>{price:,}</e>'
                            )

                            logger.info(
                                f'{self.session_name} | Загальний профіт: <e>{profit_per_hour:,}</e>'
                            )

                            logger.info(
                                f'{self.session_name} | Профіт дельта: <e>{profit:,}</e>'
                            )
